...
output_folder = fullfile(folder, 'output_device_10000_device2_user');
...
for i = 5879:length(files)
    %disp("starting_in_the_loop");
    ...
    lastElement = secondRow{1,end};

    % Check if the last element is 'Tilt Brush'
   ...
   % Create the output file name
    ...
    [r,c] = size(num) ;
    %class_str = string_cells{1, 4};
    class_str = [string_cells{1, 1}, '-', string_cells{1, 3}];
   ...
   % testClass="robo";
    ...
    %disp(total_time);
     ...
     for x=1:total_loop
        ...
        lowIndicesp = find(numPhone(:,1)<savedStart);
        numPhone(lowIndicesp,:) = [];

        highIndicesp = find(numPhone(:,1)>savedEnd);
    ...
    % Clear variables
    clear T secondRow lastElement data num string_cells;
...
%Calculating Frequency domain features
    ...
    %Dr = interp1(1:length(phoneZ), phoneZ, Tr);
    Dr_mc  = Dr - mean(Dr,1); 
...
% Number of peaks
    ...
    if length(phoneZ) >= 3
        ...
        numPeaks = 0; % or any other value that makes sense in your context
    end
