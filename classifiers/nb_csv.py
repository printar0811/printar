...
if len(csv_files) == 0:
    ...
    tn_counts = np.zeros(num_classes)
    for i in range(num_classes):
        tn_counts[i] = matrix.sum() - (matrix[i, :].sum() + matrix[:, i].sum() - matrix[i, i])

    tnr_per_class = tn_counts / (matrix.sum(axis=0) - np.diag(matrix) + tn_counts)
